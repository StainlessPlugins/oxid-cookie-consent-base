...
function smarty_function_oxscript($params, &$smarty)
{
    ...
    if (isset($params['add']) || isset($params['include'])) {
        $register = Registry::get(JavaScriptRegistrator::class);
        $attributes = $params;
        unset($attributes['add']);
        unset($attributes['priority']);
        unset($attributes['widget']);
        unset($attributes['inWidget']);
        ...
        if (isset($params['add'])) {
            ...
            if (method_exists($register,'addSnippetWithAttributes')) {
                $register->addSnippetWithAttributes($params['add'], $isDynamic, $attributes);
            }
        } else {
            ...
            if (method_exists($register,'addFileWithAttributes')) {
                $register->addFileWithAttributes($params['include'], $priority, $isDynamic, $attributes);
            }
        }
    } else {
        $renderer = oxNew(JavaScriptRenderer::class);
